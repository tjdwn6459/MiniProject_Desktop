    ...
    public partial class EditAccount : Page
    {
        ...
        private  void Page_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                //TxtUserPassword.Password = user.UserPassword; 오류생김
        ...
        private void BtnUpdate_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (isValid)
            {
                ...
                try
                {
                    ...
                    user.UserPassword = Commons.GetMdHash(mdHash, user.UserPassword);
                    
                    var result = Logic.DataAccess.SetUser(user);
                    if(result == 0)
                    {
                        //수정 안됨
                        LblResult.Text = "계정수정에 문제가 발생했습니다. 관리자에게 문의 바랍니다.";
                        LblResult.Foreground = Brushes.OrangeRed;
                    }
                    else
                    {
                        //정상적 수정됨
                        LblResult.Text = "정상적으로 수정 했습니다.";
