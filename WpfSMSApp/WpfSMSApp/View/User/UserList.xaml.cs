        ...
        private void BtnExportPdf_Click(object sender, RoutedEventArgs e)
        {
            ...
            if(saveDialog.ShowDialog() == true)
            {
                ...
                try
                {
                    ...
                    //0.PDf 사용 폰트 설정
                    string nanumPath = Path.Combine(Environment.CurrentDirectory, @"NanumGothic.ttf"); //파일경로
                    BaseFont nanumBase = BaseFont.CreateFont(nanumPath, BaseFont.IDENTITY_H, BaseFont.EMBEDDED);
                    var nanumTitle = new iTextSharp.text.Font(nanumBase, 20f); //20타이틀용 나눔 폰트
                    var nanumContent = new iTextSharp.text.Font(nanumBase, 12f); //12내용 나눔 폰트
                    ...
                    pdfTable.WidthPercentage = 100; //전체 사이즈 다 쓰는것
                    ...
                    Commons.ShowMessageAsync("PDF변환", "PDF 익시포트 성공했습니다.");
                }
                catch (Exception ex)
                {
